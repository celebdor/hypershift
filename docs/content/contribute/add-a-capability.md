...
- The registry operator component, nodepool controller, and KAS params check `IsImageRegistryCapabilityEnabled`.
- The NTO component and config operator consult `IsNodeTuningCapabilityEnabled`.
- Ingress operator and config operator reference `IsIngressCapabilityEnabled`.

### 4. Update the CLI Help Text

Update the `--disable-cluster-capabilities` and `--enable-cluster-capabilities` flag descriptions in [`cmd/cluster/core/create.go`](https://github.com/openshift/hypershift/blob/main/cmd/cluster/core/create.go) to include the new value in the supported values list.
...
- Defaults to enabled when a nil `Capabilities` is provided.
- Is explicitly disabled when listed in `Capabilities.Disabled`.
- Remains enabled when not present in the disabled list.
